...
**DiGi.Geometry.PointCloud** is a C# engineering and architectural software library suite designed for point cloud processing, CAD/BIM integrations, and 3D spatial calculations.
...
* **[DiGi.Geometry.PointCloud](DiGi.Geometry.PointCloud)** (Path: `DiGi.Geometry.PointCloud\DiGi.Geometry.PointCloud`)
...
All calculations, queries, transformations, and object mutations MUST be implemented as static extension methods inside partial classes under the appropriate directory:
* **`Query`** (`/Query`): Reading properties, spatial queries, metrics calculation (does NOT mutate state).
* **`Modify`** (`/Modify`): Modifying state/properties of existing objects.
* **`Create`** (`/Create`): Creating completely new objects from input parameters.
* **`Convert`** (`/Convert`): Transforming objects between representations or target frameworks.
